...
# ── Read Receipts ─────────────────────────────────────────────────────────────
# Tracking pixel: a 1x1 transparent GIF served at /track/{id}.png
# When the email recipient opens the email, their email client loads this URL.
# We record the first load time in Supabase (track_receipts table).
# The extension polls /track/{id}/status to check if the email was opened.
#
# Supabase table: track_receipts
#   id         text PRIMARY KEY   -- the trackId generated by content.js
#   opened_at  timestamptz        -- null until first pixel load
...
_PIXEL_GIF = (
    ...
    b'\x80\x00\x00'               # GCT flag, 2 colours
    b'\xff\xff\xff'               # colour 0: white
    b'\x00\x00\x00'               # colour 1: black
    b'\x21\xf9\x04\x00\x00\x00\x00\x00'  # graphic control
...
async def track_pixel(track_id: str, r: str = "", request: Request = None):
    ...
    # Return pixel — no caching so every open registers
...
async def track_status(track_id: str, user: dict = Depends(get_current_user)):
    """
    Returns whether the tracked email has been opened.
    Called by the extension background script while polling.
    """
    import re as _re
    if not _re.match(r'^[0-9a-f]{24}$', track_id):
